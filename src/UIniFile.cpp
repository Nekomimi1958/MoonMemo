...
TStringList * UsrIniFile::AddSection(UnicodeString sct)
{
	...
	SectionList->AddObject(sct, slst);
	return slst;
}
//---------------------------------------------------------------------------
...
void UsrIniFile::LoadGridColWidth(TStringGrid *gp, int cnt, ...)
{
	...
	for (int i=0; i<cnt; i++) {
		...
		if (i<gp->FixedCols)
			...
			gp->ColWidths[i] = ReadInteger(gp->Name, "ColWidth"+IntToStr(i), arg);
	}
	va_end(ap);
}
//---------------------------------------------------------------------------
...
void UsrIniFile::SaveGridColWidth(TStringGrid *gp)
{
	for (int i=0; i<gp->ColCount; i++)
		WriteInteger(gp->Name, "ColWidth" + IntToStr(i), gp->ColWidths[i]);
}
...
// force : 変更がなくても強制的に保存
//---------------------------------------------------------------------------
bool UsrIniFile::UpdateFile(bool force)
{
	std::unique_ptr<TStringList> fbuf(new TStringList());
	try {
		if (Modified || force) {
